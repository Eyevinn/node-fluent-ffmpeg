...
module.exports = function(proto) {
  ...
  proto._spawnFfmpeg = function(args, options, processCB, endCB) {
    ...
    this._getFfmpegPath(function(err, command) {
      ...
      if (options.measureCpu && !utils.isWindows) {
        args.unshift('-o', options.measureCpu.output, '-f', options.measureCpu.format, command);
        command = options.measureCpu.timeCmd || '/usr/bin/time';
      }
    ...
    self._prepare(function(err, args) {
      ...
      self._spawnFfmpeg(
        args,
        {
          ...
          measureCpu: self.options.measureCpu
        },
